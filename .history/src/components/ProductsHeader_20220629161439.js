...
const ProductsHeader = ({ products, setProducts, checkedState }) => {
  const handleRemove = () => {
    ...
    for (let i = 0, p = Promise.resolve(); i < selectedIds.length; i++) {
      p = p
        .then(response => productService.remove(selectedIds[i]))
        .then(console.log('Removed', selectedIds[i]))
    }

    /* Promise.all(selectedIds.forEach(id => productService.remove(id))).then(
      result => setProducts(remainingProducts)
    ) */

    //productService.remove('6')
