...
const ProductForm = () => {
  ...
  const allNames = Joi.array().items(nameSchema).unique('name')

  const validate = () => {
    const objectResult = Joi.validate(newProduct, objectSchema, {
      abortEarly: false,
    })
    ...
    if (params.id === 'new') {
      ...
      productsArray = [{ name: 'test' }, { name: 'test' }]
    } else {
      productsArray = [...products]
    }

    console.log(productsArray)
    const arrayResult = Joi.validate(productsArray, allNames, {
      abortEarly: false,
    })
    ...
    if (arrayResult.error !== null) {
      console.log(
        'arrayResult.error.details',
        arrayResult.error === null ? null : arrayResult.error.details
      )
