...
const ProductForm = () => {
  ...
  const validate = () => {
    ...
    if (!name) customErrors.concat({ name: 'name field is missing' })
    if (!description)
      customErrors.concat({ description: 'description field is missing' })
    if (!price) customErrors.concat({ price: 'price field is missing' })
